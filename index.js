...
function teamClick(d) {
    ...
    function createPieChart() {
        ...
        teamPlayer = [];
        //teamPlayerName for color return;
        teamPlayerName = [];

        d3.csv("data/players2.csv", function(error, playerData) {
            ...
            for (var i = 0 ; i < 6; i++) {
                ...
                var attrName = ["PTS", "AST", "REB", "BLK", "STL", "TOV"];
                ...
                pieChart.attr("class", "pie-chart0").attr("transform", function() { 
                    if (i <= 2){
                        return "translate(" + 770 + "," + (100 + i * 160) +")" ;
                    } else {
                        return "translate(" + 930 + "," + (100 + (i - 3) * 160) +")" ;
                    }
                });
                ...
                pieChart.append("g:text")
                    ...
                    .text(function() { return attrName[i]; })
                    .call(tip); 
...
function renderRadarChart() {
    var teamRadar = svg.selectAll("g.teamRadar").data(
        [
            teamDataset(teamRadarData)
        ]
    );
...
function renderLengend(teamList) {
    svg.selectAll('.legend-tag').remove();
    var colorscale = d3.scale.category10();
    //Initiate Legend   
    var legend = svg.select(".teamRadar").selectAll("g.legend-tag").data(teamList).enter().append("g")
